...
public class RunWhisper : MonoBehaviour
{
    ...
    [Tooltip("Put an audio to run a dummy inference")]
    [SerializeField] private AudioClip dummyClip;
    ...
    public async void Start()
    {
        ...
        audioInput = new Tensor<float>(new TensorShape(1, maxSamples));
        ComputeTensorData.Pin(audioInput);

        tokensTensor = new Tensor<int>(new TensorShape(1, maxTokens));
        ComputeTensorData.Pin(tokensTensor);

        await Transcribe(dummyClip);
        outputString = "";
    }

    public async Awaitable<string> Transcribe(AudioClip audioClip)
    {
    ...
    public async Awaitable<string> Transcribe(float[] data)
    {
        lastToken = new NativeArray<int>(1, Allocator.Persistent); lastToken[0] = NO_TIME_STAMPS;
        lastTokenTensor = new Tensor<int>(new TensorShape(1, 1), new[] { NO_TIME_STAMPS });
    ...
    void LoadAudio(float[] data)
    {
        numSamples = data.Length;
        float[] dataMax = new float[maxSamples];
        if (numSamples < maxSamples)
        {
            Array.Copy(data, dataMax, data.Length);

            audioInput.Upload(dataMax);
        }
        else if (numSamples == maxSamples)
        {
            audioInput.Upload(data);
        }
        else
        {
            throw new Exception("Audio duration should last a maximum of 30s with 16khz sample rate.");
        }
    }
